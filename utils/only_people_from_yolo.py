...
def has_person_in_txt(
    ID_CLASSES_PERSON_BEFORE: tuple, file_names, dir_path: str, storage_type=list
):
                ...
                for particular_word in ID_CLASSES_PERSON_BEFORE:
                    if line.startswith(particular_word + " "):
                        correct_lines.append(
                            f"{str(ID_CLASS_PERSON_NEW)}{line.lstrip(particular_word)}"
                        )  # пробел из правой части остается
